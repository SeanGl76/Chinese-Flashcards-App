...
export async function handleUserSend() {
    ...
    try {
        ...
        INSTRUCTIONS:
        1. Check the user's input for grammar mistakes. 
        2. If there is a mistake, explain it briefly in the "correction" field. If perfect, leave it empty.
        3. THEN, respond naturally to the content of the message to keep the chat going.
        4. OUTPUT PURE JSON ONLY.
        
        FORMAT (JSON):
        { 
            "correction": "Optional: 'You said X, but native speakers say Y because...'",
            "chinese": "Hanzi response (Ensure full sentence)", 
        ...
        try {
             ...
             replyObj = JSON.parse(cleanJson);
        } catch(e) {
             const jsonStart = rawJson.indexOf('{');
             const jsonEnd = rawJson.lastIndexOf('}');
             if (jsonStart !== -1 && jsonEnd !== -1) {
                 replyObj = JSON.parse(rawJson.substring(jsonStart, jsonEnd + 1));
             } else {
                 replyObj = { chinese: rawJson, pinyin: "", english: "" };
             }
        }
        ...
        chatHistory.push({ role: "assistant", content: rawJson });
