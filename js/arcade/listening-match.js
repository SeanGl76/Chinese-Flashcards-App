...
export function loadListeningRound() {
    ...
    wrapper.style.cssText = "flex: 1; display: flex; flex-direction: column; width: 100%; position: relative; overflow: hidden;justify-content: center;";
    // Reduced padding-top to 40px to fit End Button comfortably
    container.style.cssText = "flex: 1; width: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; overflow-y: auto; gap: 15px;box-sizing: border-box;";
...
export function checkListeningAnswer(btn, selected, correct) {
    ...
    if (selected.hanzi === correct.hanzi) {
        ...
        document.querySelector('.app-container').classList.add('shake-screen');
        setTimeout(() => document.querySelector('.app-container').classList.remove('shake-screen'), 500);
